...
export class NewsSapiensEntity implements NewsSourceInterface {
    id: string;
    name: string;
    url: string;
    active: boolean;
    ...
    loadNews(node: Node): void {
        ...
        newsNodes.forEach((_newsNode) => {
            ...
            let url = '';
            // Title
            const titleLinks: Node[] = this.newstrackerService.findNodesWithClassAttr(_newsNode, 'c-news-block__title');
            if (titleLinks.length>0) {
                let titleNode: Node = titleLinks[0];
                if (titleNode.children && titleNode.children.length>0 && typeof titleNode.children[0]==='object' && (titleNode.children[0] as Node).tag==='a') {
