...
export class NewsMirrorEntity implements NewsSourceInterface {
    id: string;
    name: string;
    url: string;
    active: boolean;
    ...
    loadNews(node: Node): void {
        ...
        newsNodes.forEach((_newsNode) => {
            ...
            if (titleLinks.length>0) {
                let titleNode: Node = this.newstrackerService.findNodesWithClassAttr(titleLinks[0], 'story__title')[0];
                if (titleNode.children && titleNode.children.length>0) {
                    title = titleNode.tag==='h2' ? titleNode.children[0] as string : '';
                    if (titleNode.tag==='div' && titleNode.children && (titleNode.children[0] as Node).tag==='h2') {
                ...
                if (linkNodes.length>0) {
                    url = this.newstrackerService.getNodeAttr(linkNodes[0], 'href');
                }
            }
            // Date
            ...
            if (summaryNodes.length===1 && summaryNodes[0].children && summaryNodes[0].children.length>0) {
                if (typeof summaryNodes[0].children[0] === 'object' && summaryNodes[0].children[0].children) {
                    content = summaryNodes[0].children[0].children[0].toString();
                }
            }
            // Image
