...
export class NewsFotogramasEntity implements NewsSourceInterface {
    id: string;
    name: string;
    url: string;
    active: boolean;
    ...
    loadNews(node: Node): void {
        this.news = [];
        const newsNodes: Node[] = this.newstrackerService.findNodesWithAttributeValue(node, 'data-theme-key', 'custom-item');
        ...
        newsNodes.forEach((_newsNode, idx) => {
            ...
            let titleNodes: Node[] = this.newstrackerService.findNodesWithAttributeValue(_newsNode, 'data-theme-key', 'custom-item-title-text');
