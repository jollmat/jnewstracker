...
export class NewsAngularLoveEntity implements NewsSourceInterface {
    id: string;
    name: string;
    url: string;
    active: boolean;
    ...
    loadNews(node: Node): void {
        this.news = [];
        const newsNodes: Node[] = this.newstrackerService.findNodesWithClassAttr(node, 'la-une').concat(this.newstrackerService.findNodesWithTag(node, 'al-article-card'));
