...
export class NewsVisitSitgesEntity implements NewsSourceInterface {
    id: string;
    name: string;
    url: string;
    active: boolean;
    ...
    loadNews(node: Node): void {
        ...
        newsNodes.forEach((_newsNode, idx) => {
            ...
            if (linkNodes.length>0 && imageNodes.length>0) {
                title = this.newstrackerService.getNodeAttr((imageNodes[0] as Node), 'title') || '';
                ...
                if (contentNodes.length>0 && contentNodes[0].children && contentNodes[0].children.length>0) {
                    ...
                    content = contentNode.children? contentNode.children[0] as string : '';
                }
            }
